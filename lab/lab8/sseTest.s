...
.LFB682:
	...
	mov	edi, 1
	movsd	QWORD PTR [rsp+32], xmm0				# 2
	mov	eax, 6
	movsd	QWORD PTR [rsp+64], xmm6				# 4
	movsd	QWORD PTR [rsp+72], xmm6				# 4.5
	movsd	QWORD PTR [rsp+80], xmm6				# 5
	movsd	QWORD PTR [rsp+88], xmm6				# 5.5
	movsd	xmm1, QWORD PTR .LC2[rip]
	movsd	QWORD PTR [rsp], xmm6
	movupd	xmm4, XMMWORD PTR [rsp+32]
	movapd	xmm5, XMMWORD PTR .LC5[rip]
	mulpd	xmm5, xmm4								# m5 * m4 -> m4
	movsd	QWORD PTR [rsp+48], xmm1
	movupd	xmm3, XMMWORD PTR [rsp+64]
	movupd	xmm2, XMMWORD PTR [rsp+80]
	addpd	xmm3, xmm5								# m3 + m5 -> m5
	movsd	QWORD PTR [rsp+16], xmm7
	xorpd	xmm5, xmm5
	mulpd	xmm4, xmm5								# m4 * m5 -> m5
	addpd	xmm2, xmm4								# m2 + m4 -> m4
